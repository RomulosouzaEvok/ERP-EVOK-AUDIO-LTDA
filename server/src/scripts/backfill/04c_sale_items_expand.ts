/**
 * Fase 4.3b: Backfill sale_items.item_id
 *
 * Script transacional por lotes (5000 registros/lote):
 * - Busca registros de sale_items com item_id NULL
 * - Mapeia product_id → item_id via crosswalk migracao_product_item_map
 * - UPDATE sale_items SET item_id = mapped_item_id
 * - Rollback automático por lote se falhar
 *
 * Uso: npx tsx server/src/scripts/backfill/04c_sale_items_expand.ts [--start 0] [--limit 50000]
 */
...
/**
 * Processa um lote de sale_items (transação isolada)
 */
async function processLote(
  ...
  try {
    // 1. Buscar lote de sale_items com item_id NULL
    ...
    for (const row of rows) {
      ...
      if (mapping.length > 0) {
        ...
        await sequelize.query(
          `UPDATE sale_items SET item_id = :item_id WHERE id = :id`,
          {
...
/**
 * Executa backfill de sale_items.item_id via crosswalk
 */
async function backfillSaleItemsExpand(opts: { start?: number; limit?: number } = {}): Promise<void> {
  const startOffset = opts.start || 0;
  const limit = opts.limit || Infinity;

  console.log('🚀 Iniciando Fase 4.3b: Backfill sale_items.item_id (expand-contract)');
  ...
  try {
    ...
    const totalCount: any = await sequelize.query(
      'SELECT COUNT(*) as count FROM sale_items WHERE item_id IS NULL',
      {
        type: 'SELECT' as any,
      }
    );
    ...
    // Resumo final
    console.log('\n' + '='.repeat(70));
    console.log('📊 RESUMO DO BACKFILL (Fase 4.3b)');
    ...
    if (stats.backfilledRows + stats.skippedRows === stats.totalRows) {
      ...
      console.log('\n📌 Próximo: Fase 4.3c (Validação SQL)');
    } else {
...
if (require.main === module) {
  ...
  backfillSaleItemsExpand(opts);
}

export { backfillSaleItemsExpand, BackfillStats };
