/**
 * Fase 4.4b: Backfill production_orders.item_id
 *
 * Script transacional por lotes (5000 registros/lote):
 * - Busca registros de production_orders com item_id NULL
 * - Mapeia product_id → item_id via crosswalk migracao_product_item_map
 * - UPDATE production_orders SET item_id = mapped_item_id
 * - Rollback automático por lote se falhar
 *
 * Uso: npx tsx server/src/scripts/backfill/04d_production_orders_expand.ts [--start 0] [--limit 50000]
 */
...
/**
 * Processa um lote de production_orders (transação isolada)
 */
async function processLote(
  ...
  try {
    // 1. Buscar lote de production_orders com item_id NULL
    const rows: any[] = await sequelize.query(
      `SELECT id, product_id FROM production_orders
    ...
    for (const row of rows) {
      ...
      if (mapping.length > 0) {
        ...
        await sequelize.query(
          `UPDATE production_orders SET item_id = :item_id WHERE id = :id`,
          {
...
/**
 * Executa backfill de production_orders.item_id via crosswalk
 */
async function backfillProductionOrdersExpand(opts: { start?: number; limit?: number } = {}): Promise<void> {
  const startOffset = opts.start || 0;
  const limit = opts.limit || Infinity;

  console.log('🚀 Iniciando Fase 4.4b: Backfill production_orders.item_id (expand-contract)');
  ...
  try {
    ...
    const totalCount: any = await sequelize.query(
      'SELECT COUNT(*) as count FROM production_orders WHERE item_id IS NULL',
      {
        type: 'SELECT' as any,
      }
    );
    ...
    // Resumo final
    console.log('\n' + '='.repeat(70));
    console.log('📊 RESUMO DO BACKFILL (Fase 4.4b)');
    ...
    if (stats.backfilledRows + stats.skippedRows === stats.totalRows) {
      ...
      console.log('\n📌 Próximo: Fase 4.4c (Validação SQL)');
    } else {
...
if (require.main === module) {
  ...
  backfillProductionOrdersExpand(opts);
}

export { backfillProductionOrdersExpand, BackfillStats };
