/**
 * Fase 4.7b: Backfill serial_numbers.item_id
 *
 * Script transacional por lotes (5000 registros/lote):
 * - Busca registros de serial_numbers com item_id NULL
 * - Mapeia product_id → item_id via crosswalk migracao_product_item_map
 * - UPDATE serial_numbers SET item_id = mapped_item_id
 * - Rollback automático por lote se falhar
 *
 * Uso: npx tsx server/src/scripts/backfill/04g_serial_numbers_expand.ts [--start 0] [--limit 50000]
 */
...
/**
 * Processa um lote de serial_numbers (transação isolada)
 */
async function processLote(
  ...
  try {
    // 1. Buscar lote de serial_numbers com item_id NULL
    ...
    for (const row of rows) {
      ...
      if (mapping.length > 0) {
        ...
        await sequelize.query(
          `UPDATE serial_numbers SET item_id = :item_id WHERE id = :id`,
          {
...
/**
 * Executa backfill de serial_numbers.item_id via crosswalk
 */
async function backfillSerialNumbersExpand(opts: { start?: number; limit?: number } = {}): Promise<void> {
  const startOffset = opts.start || 0;
  const limit = opts.limit || Infinity;

  console.log('🚀 Iniciando Fase 4.7b: Backfill serial_numbers.item_id (expand-contract)');
  ...
  try {
    ...
    const totalCount: any = await sequelize.query(
      'SELECT COUNT(*) as count FROM serial_numbers WHERE item_id IS NULL',
      {
        type: 'SELECT' as any,
      }
    );
    ...
    // Resumo final
    console.log('\n' + '='.repeat(70));
    console.log('📊 RESUMO DO BACKFILL (Fase 4.7b)');
    ...
    if (stats.backfilledRows + stats.skippedRows === stats.totalRows) {
      ...
      console.log('\n📌 Próximo: Fase 4.7c (Validação SQL)');
    } else {
...
if (require.main === module) {
  ...
  backfillSerialNumbersExpand(opts);
}

export { backfillSerialNumbersExpand, BackfillStats };
