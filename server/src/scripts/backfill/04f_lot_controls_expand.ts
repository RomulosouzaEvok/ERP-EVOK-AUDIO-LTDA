/**
 * Fase 4.6b: Backfill lot_controls.item_id
 *
 * Script transacional por lotes (5000 registros/lote):
 * - Busca registros de lot_controls com item_id NULL
 * - Mapeia product_id → item_id via crosswalk migracao_product_item_map
 * - UPDATE lot_controls SET item_id = mapped_item_id
 * - Rollback automático por lote se falhar
 *
 * Uso: npx tsx server/src/scripts/backfill/04f_lot_controls_expand.ts [--start 0] [--limit 50000]
 */
...
/**
 * Processa um lote de lot_controls (transação isolada)
 */
async function processLote(
  ...
  try {
    // 1. Buscar lote de lot_controls com item_id NULL
    const rows: any[] = await sequelize.query(
      `SELECT id, product_id FROM lot_controls
    ...
    for (const row of rows) {
      ...
      if (mapping.length > 0) {
        ...
        await sequelize.query(
          `UPDATE lot_controls SET item_id = :item_id WHERE id = :id`,
          {
...
/**
 * Executa backfill de lot_controls.item_id via crosswalk
 */
async function backfillLotControlsExpand(opts: { start?: number; limit?: number } = {}): Promise<void> {
  const startOffset = opts.start || 0;
  const limit = opts.limit || Infinity;

  console.log('🚀 Iniciando Fase 4.6b: Backfill lot_controls.item_id (expand-contract)');
  ...
  try {
    ...
    const totalCount: any = await sequelize.query(
      'SELECT COUNT(*) as count FROM lot_controls WHERE item_id IS NULL',
      {
        type: 'SELECT' as any,
      }
    );
    ...
    // Resumo final
    console.log('\n' + '='.repeat(70));
    console.log('📊 RESUMO DO BACKFILL (Fase 4.6b)');
    ...
    if (stats.backfilledRows + stats.skippedRows === stats.totalRows) {
      ...
      console.log('\n📌 Próximo: Fase 4.6c (Validação SQL)');
    } else {
...
if (require.main === module) {
  ...
  backfillLotControlsExpand(opts);
}

export { backfillLotControlsExpand, BackfillStats };
