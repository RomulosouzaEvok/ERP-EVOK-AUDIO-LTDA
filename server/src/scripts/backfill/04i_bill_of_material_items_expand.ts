/**
 * Fase 4.9b: Backfill bill_of_material_items.item_id
 *
 * Script transacional por lotes (5000 registros/lote):
 * - Busca registros de bill_of_material_items com item_id NULL
 * - Mapeia component_product_id → item_id via crosswalk migracao_product_item_map
 * - UPDATE bill_of_material_items SET item_id = mapped_item_id
 * - Rollback automático por lote se falhar
 *
 * Uso: npx tsx server/src/scripts/backfill/04i_bill_of_material_items_expand.ts [--start 0] [--limit 50000]
 */
...
/**
 * Processa um lote de bill_of_material_items (transação isolada)
 */
async function processLote(
  ...
  try {
    // 1. Buscar lote de bill_of_material_items com item_id NULL
    const rows: any[] = await sequelize.query(
      `SELECT id, component_product_id FROM bill_of_material_items
    ...
    for (const row of rows) {
      const mapping: any = await sequelize.query(
        `SELECT item_id FROM migracao_product_item_map
         WHERE product_id = :component_product_id AND status = 'SUCESSO'`,
        {
          replacements: { component_product_id: row.component_product_id },
          type: 'SELECT' as any,
          transaction,
        }
      );
      ...
      if (mapping.length > 0) {
        ...
        await sequelize.query(
          `UPDATE bill_of_material_items SET item_id = :item_id WHERE id = :id`,
          {
...
/**
 * Executa backfill de bill_of_material_items.item_id via crosswalk
 */
async function backfillBillOfMaterialItemsExpand(opts: { start?: number; limit?: number } = {}): Promise<void> {
  const startOffset = opts.start || 0;
  const limit = opts.limit || Infinity;

  console.log('🚀 Iniciando Fase 4.9b: Backfill bill_of_material_items.item_id (expand-contract)');
  ...
  try {
    ...
    const totalCount: any = await sequelize.query(
      'SELECT COUNT(*) as count FROM bill_of_material_items WHERE item_id IS NULL',
      {
        type: 'SELECT' as any,
      }
    );
    ...
    // Resumo final
    console.log('\n' + '='.repeat(70));
    console.log('📊 RESUMO DO BACKFILL (Fase 4.9b)');
    ...
    if (stats.backfilledRows + stats.skippedRows === stats.totalRows) {
      ...
      console.log('\n📌 Próximo: Fase 4.9c (Validação SQL)');
    } else {
...
if (require.main === module) {
  ...
  backfillBillOfMaterialItemsExpand(opts);
}

export { backfillBillOfMaterialItemsExpand, BackfillStats };
