/**
 * Fase 4.1b: Backfill inventory_movements.item_id
 *
 * Script transacional por lotes (5000 registros/lote):
 * - Busca registros de inventory_movements com item_id NULL
 * - Mapeia product_id → item_id via crosswalk migracao_product_item_map
 * - UPDATE inventory_movements SET item_id = mapped_item_id
 * - Rollback automático por lote se falhar
 *
 * Uso: npx tsx server/src/scripts/backfill/04a_inventory_movements_expand.ts [--start 0] [--limit 50000]
 */
...
/**
 * Processa um lote de inventory_movements (transação isolada)
 */
async function processLote(
  ...
  try {
    // 1. Buscar lote de inventory_movements com item_id NULL
...
/**
 * Executa backfill de inventory_movements.item_id via crosswalk
 */
async function backfillInventoryMovementsExpand(opts: { start?: number; limit?: number } = {}): Promise<void> {
  const startOffset = opts.start || 0;
  const limit = opts.limit || Infinity;

  console.log('🚀 Iniciando Fase 4.1b: Backfill inventory_movements.item_id (expand-contract)');
  ...
  try {
    ...
    const totalCount: any = await sequelize.query(
      'SELECT COUNT(*) as count FROM inventory_movements WHERE item_id IS NULL',
      {
        type: 'SELECT' as any,
      }
    );
    ...
    // Resumo final
    console.log('\n' + '='.repeat(70));
    console.log('📊 RESUMO DO BACKFILL (Fase 4.1b)');
    ...
    if (stats.backfilledRows + stats.skippedRows === stats.totalRows) {
      ...
      console.log('\n📌 Próximo: Fase 4.1c (Validação SQL)');
    } else {
...
if (require.main === module) {
  ...
  backfillInventoryMovementsExpand(opts);
}

export { backfillInventoryMovementsExpand, BackfillStats };
