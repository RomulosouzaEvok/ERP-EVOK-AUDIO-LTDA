-- Fase 4.2c: Valida��o de Backfill purchase_order_items.item_id
-- Executar ap�s backfill 04b com sucesso
...
LEFT JOIN items ON poi.item_id = items.id
...
WITH verificacao AS (
  SELECT
    ...
    CASE WHEN poi.item_id = m.item_id THEN 'OK' ELSE 'MISMATCH' END as status
  FROM purchase_order_items poi
  LEFT JOIN migracao_product_item_map m ON poi.product_id = m.product_id
  WHERE poi.item_id IS NOT NULL
)
SELECT
...
-- ============================================================================
-- BLOCO 5: DISTRIBUI��O POR STATUS
-- ============================================================================
