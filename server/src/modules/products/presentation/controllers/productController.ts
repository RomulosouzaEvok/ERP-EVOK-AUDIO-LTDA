...
exports.movement = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const { product_id, type, quantity, warehouse_code, description } = parsed.data;
    ...
    const { movement, product, previousQuantity, newQuantity } = await useCase.execute({
      product_id, type, quantity, warehouse_code, description, userId: req.user.id, transaction: t
    });
