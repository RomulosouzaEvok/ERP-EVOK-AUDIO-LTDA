/**
 * `GET /api/jur/contracts/:id/addendums` — histórico imutável de aditivos.
 *
 * @module modules/juridico/application/use-cases/contract/ListContractAddendumsUseCase
 */
...
class ListContractAddendumsUseCase extends UseCase<{ contractId: number | string }, any[]> {
  ...
  public async execute({ contractId }: { contractId: number | string }): Promise<any[]> {
    ...
    return this.repository.listAddendums(contractId);
  }
}

export = ListContractAddendumsUseCase;
