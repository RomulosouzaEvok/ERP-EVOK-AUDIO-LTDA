...
const Sale = sequelize.define('Sale', {
  ...
  discount: { type: DataTypes.DECIMAL(10, 2), allowNull: false, defaultValue: 0, comment: 'Desconto concedido' },
  status: { type: DataTypes.ENUM('quote', 'confirmed', 'partially_invoiced', 'invoiced', 'shipped', 'canceled'), allowNull: false, defaultValue: 'quote' },
  payment_method: { type: DataTypes.ENUM('cash', 'credit_card', 'debit_card', 'pix', 'boleto', 'transfer'), allowNull: false, defaultValue: 'pix' },
  installments: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 1, comment: 'Número de parcelas' },
  notes: { type: DataTypes.TEXT, allowNull: false, defaultValue: '' },
  nfe_number: { type: DataTypes.STRING(50), allowNull: true, comment: 'Nº da NF-e; NULL até a emissão em POST /api/sales/:id/nfe. NOT NULL indevido removido na migration 20260810-000028 (BUG-03)' },
  nfe_status: { type: DataTypes.ENUM('pending', 'processing', 'authorized', 'denied', 'cancelled'), allowNull: false, defaultValue: 'pending' },
  nfe_key: { type: DataTypes.STRING(50), allowNull: true, comment: 'Chave de acesso da NF-e; NULL até a autorização. NOT NULL indevido removido na migration 20260810-000028 (BUG-03)' },
