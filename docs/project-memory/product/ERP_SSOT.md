# ERP Evok Áudio LTDA — Single Source of Truth (SSOT) do Produto

> **Origem:** este arquivo era o `CLAUDE.md` da raiz do repositório até 2026-08-12, quando
> o CLAUDE.md foi reduzido às regras operacionais permanentes do CoreTriad (ver
> `CLAUDE.md` na raiz e `docs/coretriad/CORETRIAD_MASTER_SPEC.md`). Todo o conteúdo
> abaixo é o mesmo que estava lá, sem reescrita — só mudou de endereço. As seções §10/11
> (orquestração de agentes "Centro Autônomo de Engenharia de Software") descrevem o
> modelo que precedeu o CoreTriad e ficam aqui como registro histórico da transição;
> o modelo vigente de organização de agentes é o CoreTriad (OpusCore/VeriCore/SanaCore),
> não mais o descrito nessas duas seções.
...
### Cobertura
- **RH & Departamentos:** Estrutura organizacional, funcionários, admissão/demissão, contratos de experiência, férias, afastamentos, benefícios, treinamentos e (NOVO, 2026-08-12) importador de ponto eletrônico via AEJ (Arquivo Eletrônico de Jornada, Portaria MTP 671/2021) — backend + 10 abas web (sem folha própria; ponto é INTEGRADO, não administrado — ver `docs/rh/04-FREQUENCIA.md`)
...
- ✅ Database: PostgreSQL 16 — **📏 MEDIÇÃO CANÔNICA (2026-08-14, contada em `erp_evok_audio_test`): 170 migrations aplicadas · 209 tabelas · 480 foreign keys.** Esta é a **única** linha do projeto que carrega esses números; qualquer outro documento deve apontar para cá em vez de repeti-los (a repetição foi a causa de 3 contagens divergentes convivendo até 2026-08-11). A 168ª é `20260812-000046-create-directorate-governance.cjs` (módulo Diretoria — `strategic_plannings`/`meeting_minutes`/`business_risks`, 3 tabelas e 7 foreign keys novas); a 169ª é `20260812-000047-hr-absences-open-unique.cjs` (índice único parcial contra corrida de afastamento duplicado, achado de auditoria — sem tabela/FK nova). A 170ª é `20260814-000048-jur-approval-thresholds-and-authority-find-erp-005.cjs` (remediação SanaCore do `FIND-ERP-005`, caso `ERP-LEGACY-001-CASE-002`: tabelas `jur_approval_thresholds` e `jur_approval_threshold_history`, +2 foreign keys, e troca da unicidade por papel por unicidades que cobrem a identidade do aprovador) — **aplicada em `erp_evok_audio_test`** (✔ aplicada — conferido contra `SequelizeMeta` em 2026-08-14); nasce na branch `sana/ERP-LEGACY-001/FIND-ERP-005`, e enquanto essa branch não for integrada os dois bancos **divergem por ela** (a guarda `cross-database-drift-guard` acusa a diferença corretamente). Os dois bancos eram **idênticos** — auditoria de QA de 2026-08-12 encontrou os dois divergindo por 2 migrations (aplicadas só no banco de teste) logo após o pull destes commits; corrigido no mesmo dia rodando `migration:up` contra `erp_evok_audio`. O baseline passou a ser **DDL estático congelado** (`server/database/postgresql/00_baseline_frozen.sql`) em vez de gerado a partir dos models — banco novo nasce igual ao atual. Ver `docs/database/DATABASE.md`, seção *"Baseline congelado"*
...
- ✅ **4 bloqueadores P0 + 2 P1 remediados em 2026-08-02** (commit `d1d3aff`) — Veja [AUDITORIA_PRE_PRODUCAO_2026-08-02.md](../../governance/auditorias/AUDITORIA_PRE_PRODUCAO_2026-08-02.md)
...
- ✅ **Achados da auditoria VeriCore de 2026-08-12** (`AUD-2026-08-ERP-EVOK-FULL`, ver `audit/runs/AUD-2026-08-ERP-EVOK-FULL/`): `/uploads` estava público sem autenticação (corrigido — exige JWT), validação de magic bytes era no-op em todo upload (corrigido — deriva mime da whitelist de extensão), `maintenance` não auditava escrita real (corrigido — `logAction` adicionado), `hr_absences` sem trava de concorrência (corrigido — índice único parcial). Findings abertos: regressão de performance real em `GET /api/traceability/items/:id` (fan-out de JOIN sem `separate:true`, 12.6s medidos — `AUD-QA-002`) mascarando 2 falhas de timeout em cascata na suíte de integração (`AUD-QA-001`); contagem de teste do runbook (§6 abaixo) desatualizada frente à execução real do mesmo dia (`AUD-QA-004`).
...
│   ├── project-memory/              # fonte de verdade do CoreTriad (PRD, ADRs, security) — este arquivo mora aqui
│   ├── control-plane/tasks/         # rastreamento leve de tarefas/handoffs entre agentes
│   ├── coretriad/                   # Constituição do CoreTriad — ver CORETRIAD_MASTER_SPEC.md
│   └── ...
├── .claude/
│   ├── agents/                      # Agentes das organizações do CoreTriad (OpusCore/VeriCore)
│   └── commands/                    # /new-feature, /architecture-review, /implement, /qa-gate, /security-gate, /release, /audit-*
├── audit/                           # VeriCore — framework, standards, templates, runs
├── organizations/                   # Governança/skills/workflows por organização do CoreTriad
├── remediation/                     # SanaCore (reservado)
├── package.json
└── CLAUDE.md                        # Regras operacionais permanentes do CoreTriad (curto)
```
...
Veja [AUDITORIA_PRE_PRODUCAO_2026-08-02.md](../../governance/auditorias/AUDITORIA_PRE_PRODUCAO_2026-08-02.md) e [LEVANTAMENTO_ERP_2026-08-02.md](../../governance/auditorias/LEVANTAMENTO_ERP_2026-08-02.md).
...
npm run test:integration   # 247 testes / 59 suítes contra PostgreSQL real, sem skips — medido 2026-08-12
                            # (2 falhas conhecidas e reais por regressão de performance em
                            # traceability-and-audit-log-regression.test.ts — ver AUD-QA-001/002
                            # em audit/runs/AUD-2026-08-ERP-EVOK-FULL/21-findings/)
...
e confirmado independentemente pela auditoria VeriCore de 2026-08-12
(`audit/runs/AUD-2026-08-ERP-EVOK-FULL/21-findings/AUD-DB-*.md`).
...
- **[docs/projeto/01-PLANO.md](../../projeto/01-PLANO.md)** — Arquitetura, modelos, stack, roadmap técnico (o doc descreve os 18 modelos do núcleo original; hoje são 175 — ver §3)
- **[docs/projeto/02-PLANO_INDUSTRIAL.md](../../projeto/02-PLANO_INDUSTRIAL.md)** — 21 departamentos, BOM de auto-falante, processos produção
- **[docs/projeto/04-USE_CASES.md](../../projeto/04-USE_CASES.md)** — Casos de uso (UC-01 a UC-XX)
- **[docs/arquitetura/DOCUMENTO_DE_REQUISITOS.md](../../arquitetura/DOCUMENTO_DE_REQUISITOS.md)** — Índice executivo de Requisitos Funcionais por módulo (link para UC/rota real cada um), + RNFs por referência
- **[docs/arquitetura/REQUISITOS_NAO_FUNCIONAIS.md](../../arquitetura/REQUISITOS_NAO_FUNCIONAIS.md)** — Requisitos não funcionais (desempenho, segurança, disponibilidade, escalabilidade)
- **[docs/arquitetura/DIAGRAMA_CASOS_DE_USO_BPMN.md](../../arquitetura/DIAGRAMA_CASOS_DE_USO_BPMN.md)** — Diagrama de casos de uso + BPMN simplificado (Order-to-Cash, Purchase-to-Pay, Qualidade, Manutenção)
- **[docs/manual/00-MANUAL_DO_USUARIO.md](../../manual/00-MANUAL_DO_USUARIO.md)** — Manual do usuário (conteúdo prático completo em Vendas, Compras, Estoque/Inventário e Produção; demais módulos ainda esqueleto)
- **[docs/governance/HANDOFF_CODEX.md](../../governance/HANDOFF_CODEX.md)** — Migração Product → Item (Fase 1–4.1, backfill scripts)
- **[docs/database/DATABASE.md](../../database/DATABASE.md)** — Changelog histórico narrativo de modelagem/migrations
- **[docs/database/00-INDICE.md](../../database/00-INDICE.md)** — Documentação de dados de referência sempre-atual (Modelo Conceitual/MER, Modelo Lógico/DER, Modelo Físico/DDL, Dicionário de Dados, Acessos e Isolamento, Estruturas Programáveis, Disaster Recovery) — contagem de tabelas em §1; o "78 tabelas" que constava aqui era de 2026-08-06

### Crítico — Go-Live
- **[docs/governance/auditorias/AUDITORIA_PRE_PRODUCAO_2026-08-02.md](../../governance/auditorias/AUDITORIA_PRE_PRODUCAO_2026-08-02.md)** — 4 bloqueadores P0, 15 altos P1, plano 30h
- **[docs/governance/go-live/GO_LIVE_G6_CHECKLIST.md](../../governance/go-live/GO_LIVE_G6_CHECKLIST.md)** — Plano operacional/checklist de Go-Live, gate atual (UAT + servidor de produção pendente)
- **[docs/governance/RESIDUAIS_ABERTOS_2026-08-10.md](../../governance/RESIDUAIS_ABERTOS_2026-08-10.md)** — **Fonte de pendências abertas**, medida contra banco/código/suíte. É esta que vale quando divergir do `TODO.md`
- **[docs/governance/TODO.md](../../governance/TODO.md)** — Diário/rastreamento histórico dia a dia de tarefas/bugs/achados. **Não é fonte confiável do que ainda está aberto** (auditoria de 2026-08-10 achou ~12 alegações falsas entre as 190 caixas `- [ ]`)
- **[docs/governance/go-live/DIARIO_BORDO_GO_LIVE_G6.md](../../governance/go-live/DIARIO_BORDO_GO_LIVE_G6.md)** — Diário de bordo append-only da execução do Go-Live G6
- **[docs/arquitetura/API.md](../../arquitetura/API.md)** — Endpoints, payloads, erros
- **`audit/runs/AUD-2026-08-ERP-EVOK-FULL/`** — Primeira auditoria completa VeriCore (escopo, inventário, plano de risco, findings) — 2026-08-12

### Operacional
- **[README.md](../../../README.md)** — Setup rápido, scripts npm
- **[docs/infra/DOCKER_POSTGRES_SETUP.md](../../infra/DOCKER_POSTGRES_SETUP.md)** — Docker Compose, troubleshooting DB
- **[docs/infra/DEPLOY.md](../../infra/DEPLOY.md)** — CI/CD, produção

### Funcional por Área
- **[docs/producao/06-BOM.md](../../producao/06-BOM.md)** — Estrutura de produtos (detalhado)
- **[docs/00-ESTRUTURA_ORGANIZACIONAL.md](../../00-ESTRUTURA_ORGANIZACIONAL.md)** — 17 departamentos reais (seed) + 6 subáreas funcionais, índice por módulo
- **[docs/administrativo/04-PERFIS_ACESSO.md](../../administrativo/04-PERFIS_ACESSO.md)** — RBAC + Perfis de Acesso configuráveis (`/api/access-profiles`), 29 módulos atribuíveis
- **[docs/administrativo/05-ORGANOGRAMA_EXECUTIVO.md](../../administrativo/05-ORGANOGRAMA_EXECUTIVO.md)** — **SSOT da hierarquia** (CEO → 4 diretorias → 17 departamentos → 6 subáreas). A 4ª diretoria (Suprimentos & Logística: Compras + Almoxarifado + Expedição) foi criada em 2026-08-11; cargo previsto, ainda vago. `client/src/lib/departments.ts` espelha esta estrutura e a guarda `departments.seeds.test.ts` reprova se divergir de `server/src/config/seeds.ts`
- Mais em `docs/comercial/`, `docs/financeiro/`, `docs/qualidade/`, etc.

### Arquitetura, Diagramas e Manual do Usuário (NOVO, 2026-08-06)
- **[docs/arquitetura/REQUISITOS_NAO_FUNCIONAIS.md](../../arquitetura/REQUISITOS_NAO_FUNCIONAIS.md)** — desempenho, segurança, disponibilidade, escalabilidade, observabilidade (extraído do código real, sem números inventados)
- **[docs/arquitetura/DIAGRAMA_ARQUITETURA_INFRAESTRUTURA.md](../../arquitetura/DIAGRAMA_ARQUITETURA_INFRAESTRUTURA.md)** — diagrama Mermaid do ambiente de dev real e do plano de produção (servidor ainda não adquirido)
- **[docs/arquitetura/DIAGRAMAS_SEQUENCIA.md](../../arquitetura/DIAGRAMAS_SEQUENCIA.md)** — sequência dos 3 fluxos mais críticos (venda→estoque→NF-e, requisição→RFQ→compra→recebimento, OP→apontamento→OEE)
- **[docs/arquitetura/DIAGRAMA_CASOS_DE_USO_BPMN.md](../../arquitetura/DIAGRAMA_CASOS_DE_USO_BPMN.md)** — diagrama de casos de uso (atores × módulos) e BPMN simplificado (Order-to-Cash, Purchase-to-Pay)
- **[docs/arquitetura/DIAGRAMA_CLASSES.md](../../arquitetura/DIAGRAMA_CLASSES.md)** / **[docs/arquitetura/DIAGRAMA_CLASSES_CAMADAS.md](../../arquitetura/DIAGRAMA_CLASSES_CAMADAS.md)** — diagrama de classes do backend e das camadas Clean Architecture
- **[docs/manual/00-MANUAL_DO_USUARIO.md](../../manual/00-MANUAL_DO_USUARIO.md)** — manual do usuário por módulo (esqueleto inicial, caminhos de menu reais)
- Modelo conceitual/lógico/físico, matriz de privilégios, procedures/triggers e plano de disaster recovery ficam em `docs/database/DATABASE.md` (tratado separadamente para não duplicar/conflitar)
...
## 10. Centro Autônomo de Engenharia de Software — Orquestração de Agentes (histórico — ver nota no topo do arquivo)

> Esta seção e a §11 descrevem o modelo de orquestração de agentes vigente ANTES da
> adoção do CoreTriad (2026-08-12, mesmo dia). O modelo atual é OpusCore/VeriCore/
> SanaCore — ver `docs/coretriad/CORETRIAD_MASTER_SPEC.md`. Mantido aqui, sem reescrita,
> como registro da transição.
...
- **Agentes CoreTriad (ver `docs/coretriad/CORETRIAD_MASTER_SPEC.md` para o modelo
  vigente):** `@tech-lead` para quebrar trabalho em tarefas técnicas;
  `@backend-engineer`/`@frontend-engineer` para implementação;
  `@software-architect`/`@security-architect` para decisões estruturais e threat
  modeling; `@code-reviewer`/`@qa-engineer`/`@sdet-test-automation` para o gate de
...
**Versão:** 1.6 SSOT (migrado para `docs/project-memory/product/ERP_SSOT.md` em 2026-08-12)
**Última atualização:** 12 de agosto de 2026 (cadeia do produto: **17/17** gaps fechados e endurecidos · 2 críticos de MRP corrigidos · 5 brechas de gate fechadas · baseline do schema congelado, banco reproduzível · suíte de integração real 247/247 alvo (2 falhas reais abertas, ver AUD-QA-001/002) · primeira auditoria completa VeriCore iniciada (`AUD-2026-08-ERP-EVOK-FULL`) · consolidação documental: `AGENTS.md` aposentado, 11 alegações falsas corrigidas, fonte de pendências unificada em `RESIDUAIS_ABERTOS_2026-08-10.md`; CoreTriad adotado como modelo de orquestração de agentes, substituindo o "Centro Autônomo de Engenharia de Software" das §10/11)
