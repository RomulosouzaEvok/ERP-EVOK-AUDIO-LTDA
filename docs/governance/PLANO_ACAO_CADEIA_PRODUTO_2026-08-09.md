...
- [x] **Uma única estrutura de produto governa planejamento e consumo** — G1:
      ...
      leitura, sem réplica.
- [ ] **Nenhum produto sai sem liberação de qualidade registrada, com
      evidência** — **parcial, e a metade que falta é a da saída.** Fechado no
      **lado da entrada**: G7 tornou a inspeção uma entidade
      (`quality_inspections`), liberar lote exige inspeção aprovada e grava
      ...
      só consome lote liberado. **Não fechado no lado da saída:**
      `server/src/services/saleStockService.ts` — a baixa de estoque na
      autorização da NF-e — **não consulta status de lote**. Não há gate de
      liberação de qualidade entre o produto acabado e a expedição.
