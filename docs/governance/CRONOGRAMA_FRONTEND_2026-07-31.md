...
- [x] Gate de aprovação da diretoria no Comex (G11-COMEX, 2026-08-10) cabeado na tela: bloco "Aprovação da diretoria" (`ImportApprovalGateCard.tsx`) alimentado **exclusivamente** por `GET /api/comex/import-processes/:id/approvals` (nunca inferido de efeito colateral), botão "Aprovar como Diretoria" (`POST /:id/approve`, visível só com o módulo `diretor`), botão de embarque desabilitado enquanto `approval_complete === false` (e também enquanto a consulta da alçada está carregando/falhou) com aviso âmbar explicando o que fazer, e o diálogo de embarque deixando de oferecer câmbio/frete/seguro/despesas (campos congelados na transição `draft → shipped`). Erros 422 do gate são traduzidos em linguagem de fábrica por `translateComexError` (`comexShared.ts` + teste `comexShared.test.ts`) — nada de `rule: G11-COMEX`/`missing_roles: diretor` cru na tela.
...
- [x] Tela de Roteiro de Fabricação (`/production/routes`, gap G5, 2026-08-10): lista filtrável (`GET /api/production/routes`), criação de rascunho (`POST`), edição de cabeçalho (`PUT /:id`), editor de operações com **ordinal derivado da posição** (setas ↑ ↓, "Inserir abaixo", renumeração automática antes do `PUT /:id/steps` — os erros `G5-SEQ-GAP`/`G5-SEQ-DUP` viram impossíveis pela UI), liberação com checklist de pré-requisitos (`PATCH /:id/activate`, alçada `approve`), aposentadoria (`PATCH /:id/inactivate`), nova revisão (`POST /:id/revise`) e exclusão de rascunho (`DELETE /:id`). Os 12 códigos `G5-*` são traduzidos para linguagem de fábrica em `productionRouteShared.ts`.
