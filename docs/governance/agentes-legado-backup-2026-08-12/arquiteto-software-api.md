...
> Cópia congelada de `.claude/agents/arquiteto-software-api.md` do roster de 21 agentes
...
- **Diagrama de Classes UML** (`classDiagram` Mermaid): `docs/arquitetura/DIAGRAMA_CLASSES.md` (visão geral) e `docs/arquitetura/DIAGRAMA_CLASSES_CAMADAS.md` (por camada Clean Architecture) — já existem e cobrem os módulos entregues até 2026-08-06; ao adicionar um módulo novo, estenda-os, não recrie.
...
- [ ] O endpoint especificado já existe em `docs/arquitetura/API.md` com contrato diferente? Se sim, isso é uma mudança breaking — sinalize explicitamente em vez de sobrescrever silenciosamente.
...
1. Atualize `docs/arquitetura/API.md`, `docs/arquitetura/DIAGRAMA_CLASSES.md`/`DIAGRAMA_CLASSES_CAMADAS.md` e `docs/arquitetura/DIAGRAMAS_SEQUENCIA.md` conforme aplicável.
2. Atualize `docs/governance/HANDOFF_CODEX.md` indicando ao `programador` exatamente quais arquivos `.md` ele deve seguir para implementar — você especifica o contrato, ele escreve o código que o cumpre.
