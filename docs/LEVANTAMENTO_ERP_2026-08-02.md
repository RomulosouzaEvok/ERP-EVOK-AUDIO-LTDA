...
| purchases | Pedido, recebimento parcial com NF e lote, dedup + ✅ **Cotação/RFQ multi-fornecedor RESOLVIDO em 2026-08-06** (módulo `rfq`, `/api/rfqs`, tela `/purchases/rfqs` — mapa comparativo, adjudicação gera pedido(s) e realimenta o catálogo `item_suppliers`, ver `docs/API.md` §11.1) | Devolução, histórico de preços exposto |
| suppliers | CRUD + trava de desativação | Homologação, qualificação por item, catálogo de preços, rating calculado |
| sales | Orçamento→confirmação, NF-e + ✅ **Alteração de pedido, faturamento parcial e tabela de preços por cliente RESOLVIDOS em 2026-08-06 (terceira rodada)** (`PUT /:id/items`, `sale_items.invoiced_quantity` + status `partially_invoiced`, `customer_price_lists`, ver `docs/API.md` §5/`docs/DATABASE.md`) | Histórico multi-NF-e por pedido (`Sale.nfe_*` só guarda a emissão mais recente — ver risco residual em `docs/governance/TODO.md`) |
| financial | AP/AR com baixa parcial + ✅ **centros de custo e projeção diária de fluxo de caixa RESOLVIDOS em 2026-08-06** (`cost_centers` + `cost_center_id` em AP/AR, `GET /api/finance/cashflow/projection`, ver `docs/DATABASE.md`/`docs/API.md` §6) + ✅ **Conciliação bancária (OFX) RESOLVIDA em 2026-08-06 (terceira rodada)** (`/api/finance/reconciliation/*`, `bank_statements`/`bank_statement_entries`, ver `docs/API.md` §6) | CNAB (boleto/remessa/retorno) ainda pendente — só OFX foi implementado; mapeamento automático departamento→centro de custo na AP automática ainda pendente |
...
- ✅ **Cotação/RFQ e mapa comparativo — RESOLVIDO em 2026-08-06**: módulo
  `rfq` (`server/src/modules/rfq/`), tabelas `rfqs`/`rfq_items`/
  `rfq_suppliers`/`rfq_quotes` (migration `20260806-000010`), endpoints
  `/api/rfqs` (criar avulsa ou a partir de requisição, convidar
  fornecedores, registrar cotação, `GET /:id/comparison` com mapa
  comparativo por item destacando melhor preço/prazo, `POST /:id/award`
  adjudicando por item — gera pedido(s) de compra por fornecedor vencedor
  e faz upsert no catálogo `item_suppliers`), tela `/purchases/rfqs`.
  Testado ao vivo ponta a ponta. Detalhe completo em `docs/API.md` §11.1 e
  `docs/DATABASE.md`.
...
| 9 | ~~**Relatórios de manufatura**~~ | ✅ **RESOLVIDO em 2026-08-03 (commit 8cf7938)**: /api/reports/production (WIP, aderência, refugo por etapa, lead time) + /api/reports/purchasing (fornecedor: valor, lead time, pontualidade, RNCs) + aba "Custos" (variância) + página /reports. ✅ **OEE completo RESOLVIDO em 2026-08-06**: `GET /api/reports/oee` (`GetOeeReportUseCase.ts`) — Disponibilidade (horas produzindo/horas disponíveis do calendário de turnos, fallback `capacity_hours_per_day`), Performance (tempo padrão × unidades / tempo real, capado a 100%), Qualidade (boas/(boas+refugo)), OEE = D×P×Q por centro de trabalho e agregado geral; `null` com `no_data_reason` explícito em vez de `0` enganoso. Aba OEE em `/reports`. ✅ **Downtime real RESOLVIDO em 2026-08-06 (terceira rodada)**: tabela `production_downtimes` (`POST/PUT/GET /api/production/downtimes`, bloqueio de 2ª parada aberta simultânea no mesmo centro em 2 níveis) — `GetOeeReportUseCase` agora desconta `downtime_hours` real das horas de calendário para calcular `available_hours` líquidas, com breakdown por motivo (`downtime_by_reason`). Limitação anterior (aproximação só por calendário de turnos, sem desconto de paradas reais) removida. Ver `docs/API.md` §7/`docs/DATABASE.md` | feito |
...
**Bombas latentes conhecidas** — ✅ **RESOLVIDAS em 2026-08-06** (mesmo
padrão do bug corrigido em 2026-08-02 na BOM): as 4 colunas originalmente
listadas aqui, `requisicoes_compra.aprovado_por`, `ordens_producao.criado_por`,
`movimentos_estoque.usuario_id`, `auditoria_eventos.usuario_id`
(UUID → `usuarios.id`), foram convertidas para `INTEGER` com FK real para
`users(id)` (`ON DELETE SET NULL`). **+3 bombas adicionais encontradas
nesta rodada e também resolvidas:** `requisicoes_compra.solicitante_id`,
`entradas_nf.recebido_por` (mesmo padrão, mesma migration) — e uma **bomba
real em tabela viva** (não só no schema-fantasma órfão):
`items.fornecedor_padrao_id` era UUID mas `suppliers.id` é INTEGER,
tornando o campo impossível de preencher via API; corrigido para INTEGER
com FK real para `suppliers(id)` — **breaking change de API** (ver
`docs/API.md` §3). As 12 tabelas do schema-fantasma em português onde
vivem as 6 colunas de usuário (0 linhas, 0 models, 0 uso em código vivo)
foram marcadas `DEPRECATED` via `COMMENT ON TABLE` (não removidas — ver
`docs/DATABASE.md`, seção "Tabelas órfãs do schema-fantasma em
português"). Migrations: `20260806-000040/041/042`. Guarda de regressão:
...
**Atualização 2026-08-06 (segunda rodada do dia — RFQ, centros de custo/projeção diária, OEE, desarme de bombas latentes):** item 1 (seção 2, catálogo item×fornecedor) ganha a RFQ/mapa comparativo que faltava; linha `purchases` e item 9 (seção 3) marcados `feito` para Cotação/RFQ; linha `financial` marcada `feito` para centros de custo + projeção diária (conciliação/CNAB seguem pendentes); item 9 (seção 3) marcado `feito` para OEE completo (limitação de downtime não modelado documentada, não é gap silencioso); seção "Bombas latentes conhecidas" totalmente resolvida (4 originais + 3 novas encontradas, incluindo uma bomba real em tabela viva — `items.fornecedor_padrao_id`, breaking change de API). 59 migrations no total após esta rodada. Detalhamento completo, riscos residuais e comandos de validação em `docs/governance/TODO.md`, `docs/DIARIO_BORDO_GO_LIVE_G6.md` (entrada 2026-08-06, segunda rodada) e `docs/HANDOFF_CODEX.md`.

**Atualização 2026-08-06 (terceira rodada do dia — auth refresh/Winston, mobile/TV, telas web, vendas, produção, financeiro):** linha `sales` marcada `feito` para os 3 gaps que restavam (alteração de pedido, faturamento parcial, tabela de preços por cliente — histórico multi-NF-e por pedido fica como risco residual registrado); linha `financial` ganha conciliação bancária OFX (`feito` — CNAB continua pendente, fora de escopo desta v1); item 9 (seção 3) ganha o fechamento da limitação de downtime documentada na rodada anterior (`production_downtimes`, OEE agora desconta paradas reais). Além disso, fora do escopo direto deste levantamento: `POST /api/auth/refresh` (renovação deslizante de token, fecha a pendência "JWT × painel TV sempre ligado"), logging estruturado Winston, paginação real na lista de contagens do app mobile, telas web de reatribuição de contagem cíclica e fornecedor padrão do item. 64 migrations no total após esta rodada. Detalhamento completo, riscos residuais e comandos de validação em `docs/governance/TODO.md`, `docs/DIARIO_BORDO_GO_LIVE_G6.md` (entrada 2026-08-06, terceira rodada) e `docs/HANDOFF_CODEX.md`.
