...
- **11 módulos com UI completa:** login/senha, usuários, produtos (cadastro/foto/QR/fornecedores — sem mais movimentação, ver Logística), contagem cíclica, vendas + NF-e, clientes, compras, fornecedores, OP + BOM, patrimônio, financeiro AP/AR, rastreabilidade, audit logs, **qualidade** (`/quality`: inspeção de recebimento de lotes + RNC), **logística** (`/logistics/estoque` e `/logistics/recebimento`).
