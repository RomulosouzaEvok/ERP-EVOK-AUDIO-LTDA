/**
 * Client HTTP genérico do app de TV — cópia adaptada de
 * `mobile/src/api/client.ts` (mesmo padrão de tratamento de erro, reaproveitado
 * por instrução explícita para não reinventar). Centraliza parsing do
 * envelope JSON do backend (`{ success, data }` / `{ success: false, error }`)
 * e traduz erros HTTP em mensagens para exibição discreta no painel.
 *
...
const FRIENDLY_MESSAGES: Record<number, string> = {
  ...
  403: 'Você não tem permissão para acessar este painel. Fale com o administrador do sistema.',
